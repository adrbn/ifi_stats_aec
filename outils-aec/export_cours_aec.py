...
# Windows : sortie console en UTF-8 (logs avec emojis → sinon UnicodeEncodeError cp1252).
...
def cocher_tout_checkboxes_popup(page):
    ...
    for i in range(opts.count()):
        try:
            ...
            o.click(force=True, timeout=4000); n += 1; pause("court")
        except Exception:
            continue
    if n:
        return n
    ...
    for i in range(cbs.count()):
        try:
            ...
            if "k-checked" in (c.get_attribute("class") or "") or c.is_checked():
                continue
            c.click(force=True, timeout=4000); n += 1; pause("court")
        except Exception:
            continue
    ...
    if n == 0:
        ...
        for i in range(chk.count()):
            try:
                chk.nth(i).click(force=True, timeout=4000); n += 1; pause("court")
            except Exception:
                continue
    return n
...
def _annees_defaut():
    ...
    # Début d'AEC = septembre 2022 → on part de 2022 (le parser filtre ensuite à
    # partir du 01/09/2022 via DATE_FLOOR). Sinon on rate sept–déc 2022.
    return [str(y) for y in range(2022, date.today().year + 1)]
...
def set_periods(page, years):
    ...
    #    AEC masque les périodes anciennes derrière un lien dont le libellé varie
    #    selon la version (« les plus anciennes » / « antérieures » / « précédentes »)
    #    → on tente plusieurs libellés, sinon les périodes cachées ne se chargent
    #    jamais et des cours (ex. janv–avr) manquent.
    _LIBELLES_VOIR_PLUS = (
    ...
    # ⚠ Le nœud ANNÉE (« 2026 ») est visible d'emblée, mais ses mois anciens
    #   (janv–avril) sont MASQUÉS sous « Voir les périodes les plus anciennes ».
    #   Cocher l'année ne prend alors que les mois déjà chargés → il faut d'abord
    #   TOUT charger. On déplie jusqu'à ce que l'année PRÉCÉDANT la plus ancienne
    #   voulue apparaisse (⇒ tous les mois de `oldest` sont chargés), ou jusqu'à
    #   ce qu'il n'y ait plus de bouton (tout l'historique est chargé).
    oldest = min(years)
    prev = str(int(oldest) - 1) if oldest.isdigit() else None
    for _ in range(40):
        if prev and _annee_presente(page, prev):
            break
        if not _clic_voir_plus():
            break
        pause("moyen")
...
def exporter(page, chemin: Path):
    ...
    try:
        ...
        dl = di.value
        # Sécurité EXTENSION : on force « .xlsx » à l'enregistrement (au cas où le
        # chemin cible n'en aurait pas). Si AEC proposait un autre format
        # (.xls/.csv), on garde SON extension d'origine pour ne pas mentir sur le
        # contenu — mais AEC exporte bien du .xlsx.
        if not chemin.suffix:
            src_ext = Path(dl.suggested_filename or "").suffix
            chemin = chemin.with_suffix(src_ext or ".xlsx")
        dl.save_as(str(chemin))
        log(f"✅ FICHIER → {chemin}  (suggéré par AEC : {dl.suggested_filename})")
        return chemin
    except Exception as e:
        log(f"✗ export raté : {e}")
        try:
...
def main():
    ...
    with sync_playwright() as pw:
        ...
        # 20 min pour se connecter manuellement (aucune pression ; la session
        # persiste ensuite dans le profil → les runs suivants n'en ont plus besoin).
