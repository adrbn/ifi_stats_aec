...
# Windows : force la sortie console en UTF-8 (les scripts loguent des emojis →
# sinon UnicodeEncodeError avec l'encodage cp1252 par défaut de la console).
...
ETAPES = [
    ...
    ("Contrôle qualité (incohérences de dates)", "controle_incoherences.py"),
]
FICHIER_RESULTAT = OUT_DIR / "oscar_indicateurs_cours.xlsx"
FICHIER_INCOHERENCES = OUT_DIR / "incoherences_dates.xlsx"
...
def run_pipeline(cutoff_months, annees=None, log=print, on_step=None, should_stop=None):
    ...
    # Sous-processus en UTF-8 (Windows) : évite les UnicodeEncodeError sur les
    # emojis des logs, côté enfant (écriture) comme parent (lecture).
    env["PYTHONUTF8"] = "1"
    ...
    for i, (titre, script) in enumerate(ETAPES, 1):
        ...
        log("\n" + "═" * 58)
        log(f"▶ Étape {i}/{len(ETAPES)} — {titre}")
        log("═" * 58)
        try:
            p = subprocess.Popen(
...
def main_cli():
    ...
    print("        2) Complète — reconstruction depuis sept 2022 (~7 min)")
    mode = input("Choix [1] : ").strip() or "1"
    annees = ([str(y) for y in range(2022, date.today().year + 1)] if mode == "2"
...
def main_gui():
    ...
    mode = ctk.CTkSegmentedButton(row1, values=["Rapide (année en cours)", "Complète (depuis sept 2022)"])
    ...
    # ---------- Correction ciblée (réexport par n° de classe) ----------
    fixrow = ctk.CTkFrame(app, fg_color="transparent"); fixrow.pack(fill="x", padx=24, pady=(10, 0))
    fix_btn = ctk.CTkButton(fixrow, text="🔁  Réexporter des cours par n°", height=40,
                            font=ctk.CTkFont(size=14, weight="bold"),
                            fg_color="#0f766e", hover_color="#115e59")
    fix_btn.pack(side="left", fill="x", expand=True)
    ctk.CTkLabel(app, text="Rafraîchit quelques cours précis depuis AEC (toutes périodes confondues) "
                           "et remplace uniquement ceux-là dans le cache.",
                 text_color="gray").pack(anchor="w", padx=28, pady=(2, 0))

    # ---------- Publication vers OSCAR (Pull Request) ----------
    pubrow = ctk.CTkFrame(app, fg_color="transparent"); pubrow.pack(fill="x", padx=24, pady=(10, 0))
    pub_btn = ctk.CTkButton(pubrow, text="🚀  Publier sur OSCAR (Pull Request)", height=40,
                            font=ctk.CTkFont(size=14, weight="bold"),
                            fg_color="#2563eb", hover_color="#1d4ed8")
    pub_btn.pack(side="left", fill="x", expand=True)
    ctk.CTkLabel(app, text="Propose les données fraîches au dépôt OSCAR par une PR (Adrien relit et publie).",
                 text_color="gray").pack(anchor="w", padx=28, pady=(2, 0))
    ...
    inc_btn = ctk.CTkButton(foot, text="📋 Incohérences", width=160, fg_color="gray30",
                            hover_color="gray40", command=lambda: ouvrir_chemin(FICHIER_INCOHERENCES),
                            state="normal" if FICHIER_INCOHERENCES.exists() else "disabled")
    inc_btn.pack(side="left", padx=(0, 8))
            ...
            while True:
                ...
                if item[0] == "step":
                    ...
                    if kind == "start":
                        ...
                        status.configure(text=f"Étape {i}/{len(ETAPES)} : {ETAPES[i-1][0]} …")
                    else:  # done
                        ok = rest[0]
    ...
    def set_running(on):
        ...
        pub_btn.configure(state="disabled" if on else "normal")
        fix_btn.configure(state="disabled" if on else "normal")
    ...
    def worker(n, annees, open_at_end):
        ...
        def done():
            ...
            if FICHIER_INCOHERENCES.exists():
                inc_btn.configure(state="normal")
    ...
    def start():
        ...
        if mode.get().startswith("Rapide"):
            ...
            annees = [str(y) for y in range(2022, date.today().year + 1)]      # reconstruction (début AEC = sept 2022)
    ...
    def publier():
        """Publie le cache cours vers OSCAR via une Pull Request (avec confirmation)."""
        if state["running"]:
            return
        from tkinter import messagebox
        if not messagebox.askyesno(
            "Publier sur OSCAR",
            "Proposer les données Cours fraîches au dépôt OSCAR ?\n\n"
            "• le cache et le snapshot seront mis à jour sur une branche dédiée\n"
            "• une Pull Request sera ouverte : Adrien relit puis publie\n"
            "• ton dépôt local revient ensuite à son état actuel\n\n"
            "Continuer ?",
        ):
            return
        set_running(True)
        status.configure(text="Publication vers OSCAR…")
        ...
        def worker_pub():
            ...
            try:
                p = subprocess.Popen(
                    [PY, str(ICI / "publier_oscar_pr.py")], cwd=str(ICI), env=env,
                    stdout=subprocess.PIPE, stderr=subprocess.STDOUT,
                    text=True, encoding="utf-8", errors="replace", bufsize=1)
                for line in p.stdout:
                    push(line.rstrip("\n"))
                p.wait()
                ok_pub = p.returncode == 0
            except Exception as e:  # noqa: BLE001
                push(f"❌ Publication impossible : {e}")

            def done_pub():
                status.configure(text="PR ouverte ✅" if ok_pub else "Publication échouée ❌")
                set_running(False)
            app.after(0, done_pub)

        threading.Thread(target=worker_pub, daemon=True).start()

    def reexporter_ids():
        """Réexporte des cours précis depuis AEC, puis les remplace dans le cache.

        Utile après une correction dans AEC : comme la PÉRIODE du cours a pu
        changer, on ne sait pas quelle période retirer — la recherche par n° de
        classe, elle, retrouve le cours où qu'il soit. Le cache n'est modifié
        qu'après affichage des changements et accord explicite.
        """
        if state["running"]:
            return
        from tkinter import messagebox
        dlg = ctk.CTkInputDialog(
            title="Réexporter des cours par n°",
            text="N° de classe séparés par des virgules.\n"
                 "Laisser VIDE = tous les cours de la liste d'incohérences.")
        saisie = dlg.get_input()
        if saisie is None:          # bouton Annuler
            return
        ids = ",".join(c.strip() for c in saisie.replace(";", ",").split(",") if c.strip())
        quoi = (f"Réexporter {len(ids.split(','))} cours" if ids
                else "Réexporter tous les cours de la liste d'incohérences")
        if not messagebox.askyesno(
            "Réexporter des cours par n°",
            f"{quoi} depuis AEC ?\n\n"
            "• une fenêtre Chrome s'ouvre (connexion AEC si la session a expiré)\n"
            "• tous les centres et toutes les périodes sont sélectionnés\n"
            "• les changements te seront montrés AVANT toute écriture dans le cache\n\n"
            "Continuer ?",
        ):
            return
        set_running(True)
        status.configure(text="Réexport ciblé depuis AEC…")
        logbox.delete("1.0", "end")

        env = dict(os.environ, PYTHONUNBUFFERED="1", PYTHONUTF8="1", PYTHONIOENCODING="utf-8")

        def run(script, *args):
            """Lance un script en streamant ses logs ; renvoie (code, lignes)."""
            lignes = []
            try:
                p = subprocess.Popen(
                    [PY, str(ICI / script), *args], cwd=str(ICI), env=env,
                    stdout=subprocess.PIPE, stderr=subprocess.STDOUT,
                    text=True, encoding="utf-8", errors="replace", bufsize=1)
                for line in p.stdout:
                    line = line.rstrip("\n")
        ...
        def worker_fix():
            rc, _ = run("reexport_ids.py", *(["--ids", ids] if ids else []))
            if rc != 0:
                fin("Réexport échoué ❌"); return

            push("")
            push("── Changements qui seraient appliqués au cache ──")
            rc, lignes = run("patch_cache_ids.py", "--dry-run")
            if rc != 0:
                fin("Comparaison échouée ❌"); return
            nb = sum(1 for l in lignes if l.startswith("  ✏") or l.startswith("  ➕"))
            if nb == 0:
                fin("Cache déjà à jour ✅"); return

            def appliquer():
                rc2, _ = run("patch_cache_ids.py")
                if rc2 == 0:
                    push("")
                    push("── Contrôle qualité relancé ──")
                    run("controle_incoherences.py")

                def done_fix():
                    status.configure(text="Cache corrigé ✅" if rc2 == 0 else "Patch échoué ❌")
                    if FICHIER_INCOHERENCES.exists():
                        inc_btn.configure(state="normal")
                    set_running(False)
                app.after(0, done_fix)

            def demander():
                from tkinter import messagebox as mb
                if mb.askyesno("Appliquer au cache ?",
                               f"{nb} cours seraient modifiés dans le cache "
                               "(sauvegarde automatique avant écriture).\n\n"
                               "Le détail est affiché dans les logs.\n\nAppliquer ?"):
                    status.configure(text="Application au cache…")
                    threading.Thread(target=appliquer, daemon=True).start()
                else:
                    status.configure(text="Changements NON appliqués.")
                    set_running(False)

            app.after(0, demander)

        threading.Thread(target=worker_fix, daemon=True).start()

    start_btn.configure(command=start)
    stop_btn.configure(command=stop)
    pub_btn.configure(command=publier)
    fix_btn.configure(command=reexporter_ids)
    pump()
    app.mainloop()
