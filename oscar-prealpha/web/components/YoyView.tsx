    ...
    <div className="space-y-5">
      ...
      <Panel title="Évolution annuelle" right={
        <div className="inline-flex gap-1 rounded-pill bg-neutral-100 p-[3px]">
          {(["inscriptions", "recettes"] as const).map((m) => (
            <button key={m} onClick={() => setMetric(m)}
              className={`rounded-pill px-3 py-1 text-body-sm font-medium capitalize transition-all ${metric === m ? "bg-accent-500 text-white shadow-sm" : "text-neutral-600 hover:text-neutral-900"}`}>
              {m === "inscriptions" ? "Inscriptions" : "Recettes"}
            </button>
          ))}
        </div>
      }>
        <GroupedYearBar rows={yoy.rows} metric={metric} />
      </Panel>
            ...
            <tbody>
              {yoy.rows.map((r) => (
                <tr key={r.year} className="even:bg-neutral-50 hover:bg-accent-50">
                  ...
                  <td className="px-3.5 py-2.5 text-right"><VarBadge v={r.recettesVar} /></td>
